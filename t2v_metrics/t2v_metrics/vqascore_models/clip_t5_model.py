...
class CLIPT5Model(VQAScoreModel):
    ...
    def load_images(self,
        ...
        Modified from original impl to also support PIL.Image inputs
        """
        if any(isinstance(x, str) and x.startswith(("http://", "https://")) for x in image):
        ...
        image = [x if isinstance(x, Image.Image) else self.image_loader(x) for x in image]
        if self.image_aspect_ratio == 'pad':
            image = [expand2square(img, tuple(int(x*255) for x in self.image_processor.image_mean)) for img in image]
        image = [self.image_processor.preprocess(img, return_tensors='pt')['pixel_values'][0] for img in image]
