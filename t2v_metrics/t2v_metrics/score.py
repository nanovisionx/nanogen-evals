...
from .vqascore_models.mm_utils import *
...
class Score(nn.Module):
    ...
    def forward(self,
        ...
        # Process scores - one-to-one matched pairs only (image_i, text_i)
        assert len(processed_images) == len(texts), "Number of images and texts must match for paired scoring"
        scores = self.model.forward(processed_images, texts, **kwargs)

        # # Process scores (N×N all pairs - commented out for efficiency)
